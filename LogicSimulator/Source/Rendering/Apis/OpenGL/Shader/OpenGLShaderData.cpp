...
uint32_t OpenGLShaderData::GetProgramID() {
	if (GetShader<Shader>()->IsDirty()) InitGLData();
	return this->programID;
}

void OpenGLShaderData::InitGLData() {
	if (this->programID) CleanUp();
	...
	for (ShaderType type : types) {
		...
		if (file) {
			this->shaders.insert({ type, false });
			fclose(file);
		}
	}
	...
	}
	{
		...
		while (itr != attribs.end()) {
			glBindAttribLocation(this->programID, itr->second, itr->first.c_str());
			itr++;
		}
	}
	...
	for (GLint i = 0; i < count; i++) {
		...
		glGetActiveUniform(this->programID, static_cast<GLuint>(i), 128, &length, &size, &type, name);
